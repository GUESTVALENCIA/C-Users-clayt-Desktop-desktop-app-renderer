...
function createWindow() {
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: path.join(__dirname, 'qwen-preload.js'), // Archivo preload para seguridad
      partition: 'persist:qwen' // Partición persistente
    },
    icon: path.join(__dirname, 'assets', 'icon.png') // Icono de la aplicación (si existe)
  });

  // Cargar el archivo HTML local en lugar de la URL remota
  ...
  switch(route) {
    ...
    case 'qwen-offline':
      ...
      if (fs.existsSync(qwenOfflinePath)) {
        ...
        // Si no existe, cargar la versión estándar
        await win.loadFile(path.join(__dirname, 'renderer', 'index.html'));
      }
      break;
    default:
